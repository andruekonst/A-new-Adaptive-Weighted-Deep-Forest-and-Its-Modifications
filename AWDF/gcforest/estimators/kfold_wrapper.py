...
class KFoldWrapper(object):
    ...
    def fit_transform(self, X, y, y_stratify, cache_dir=None, test_sets=None, eval_metrics=None, keep_model_in_mem=True,
        ...
        for k in range(self.n_folds):
            ...
            # check if current train set contain at least one sample for each class
            cur_y_unique = np.unique(y[train_idx])
            y_unique = np.unique(y)
            y_missing = set(y_unique) - set(cur_y_unique)
            for c in y_missing:
                ...
                if sample_weight is None:
                    tmp_weights = (sample_weight[ind_of_c] + eps) / (sum(sample_weight[ind_of_c]) + eps * ind_size)
                    ind = np.random.choice(ind_of_c, 1, p=tmp_weights)[0]
                else:
                    ind = np.random.choice(ind_of_c, 1)[0]
                train_idx = list(train_idx) + [ind]

            # fit on k-fold train
            est.fit(X[train_idx].reshape((-1, n_dims)), y[train_idx].reshape(-1),
                    cache_dir=cache_dir,
                    sample_weight=None if sample_weight is None else sample_weight[train_idx].reshape(-1))
                    # np.ndarray.reshape(-1) is like np.ndarray.flatten() (?)
