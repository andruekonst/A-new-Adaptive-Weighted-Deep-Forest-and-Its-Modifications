...
def get_windows_channel(X, X_win, des_id, nw, nh, win_x, win_y, stride_x, stride_y):
    ...
    dj = des_id % win_x
    di = des_id // win_x % win_y # des_id / win_x % win_y
    k = des_id // win_x // win_y # des_id / win_x / win_y
...
def get_windows(X, win_x, win_y, stride_x=1, stride_y=1, pad_x=0, pad_y=0):
    ...
    n, c, h, w = X.shape
    nc = win_y * win_x * c
    nh = (h - win_y) // stride_y + 1 # (h - win_y) / stride_y + 1
    nw = (w - win_x) // stride_x + 1 # (w - win_x) / stride_x + 1
